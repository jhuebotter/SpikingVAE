...
class SCNNModel(nn.Module):
    ...
    def convs(self, x):
        ...
        conv_modules = [c for c in self.conv_layers.values() if type(c) in [nn.Conv2d, nn.AvgPool2d]]
        ...
        with torch.no_grad():
            ...
            for name, layer in self.conv_layers.items():
                ...
                if type(layer) not in [LIF_sNeuron, Pooling_sNeuron]:
                    p = layer(p)
                    membrane_potentials.append(torch.zeros(p.size(), requires_grad=False, device=self.device))
        ...
        for t in range(self.steps):
            ...
            for name, layer in self.conv_layers.items():
                ...
                if type(layer) == LIF_sNeuron:
                    ...
                    LF_outs[j], total_outs[j], out = LF_Unit(layer.decay, LF_outs[j], total_outs[j], out, out_temps[j], t)
                    i += 1
                    j += 1
